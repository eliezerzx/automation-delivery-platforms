"""
Painel de Automação Keeta - NiceGUI  (v3 — visual 99food exato + lógica Keeta)
"""
...
COORD_KEYS = [
    ...
    ('editar_preco', 'Editar Preco'),
    ('campo_preco',  'Campo Preco'),
...
# ── Thread de automacao (logica Keeta -- nao alterar) ─────────────────────
def _thread_automacao():
    ...
    try:
        ...
        _push('separador')
        _push('inicio', 'Iniciando automacao Keeta...')
        ...
        for i, linha in enumerate(linhas, start=1):
            ...
            try:
                ...
                if len(partes) != 3:
                    _push('aviso', 'Linha ' + str(i) + ' -- formato invalido: ' + linha.strip())
                    falhados += 1; continue
                ...
                pyautogui.hotkey('ctrl', 'a')
                pyautogui.press('backspace')
                pyautogui.write('{:.2f}'.format(preco_base), interval=0.05)
                pyautogui.press('tab')
                pyautogui.press('backspace')
                pyautogui.write('{:.2f}'.format(preco_base), interval=0.05)
                ...
                if not _sleep_cancelavel(state['delay_env']):
                    _push('parada', 'Parado durante delay de envio.'); break
...
.dash-item.active { background:rgba(249,115,22,.1); border-left-color:#F97316; color:#F97316; font-weight:700; }
...
.nav-item.active { background:rgba(249,115,22,.1); border-left-color:#F97316; color:#F97316; font-weight:700; }
...
.file-item.selected { background:#F97316 !important; color:#0f172a !important; font-weight:700; }
...
.spinner-ring { width:62px; height:62px; border-radius:50%; border:5px solid rgba(249,115,22,.15); border-top-color:#F97316; flex-shrink:0; }
@keyframes spin { to { transform:rotate(360deg); } }
.spinner-ring.spinning { animation:spin .8s linear infinite; }
.spinner-ring.paused   { border-top-color:#ea6d10; animation:spin 2s linear infinite; }
...
.prog-fill  { background:linear-gradient(90deg,#ea6d10,#F97316); height:5px; width:0%; transition:width .4s; border-radius:4px; }
@keyframes pulse-glow { 0%,100% { box-shadow:0 4px 14px rgba(249,115,22,.3); } 50% { box-shadow:0 4px 26px rgba(249,115,22,.65); transform:translateY(-1px); } }
.btn-ini { flex:1; min-width:0; background:linear-gradient(135deg,#F97316,#ea6d10) !important; color:#0f172a !important; font-weight:800 !important; border-radius:10px !important; padding:12px !important; text-transform:none !important; font-size:14px !important; animation:pulse-glow 2s ease-in-out infinite; transition:all .2s !important; }
...
.btn-quick:hover { background:#243044 !important; color:#F97316 !important; border-color:rgba(249,115,22,.35) !important; }
...
.btn-cap-sm { flex:1; background:rgba(249,115,22,.12) !important; border:1px solid rgba(249,115,22,.4) !important; color:#F97316 !important; border-radius:7px !important; font-size:12px !important; text-transform:none !important; padding:6px 10px !important; font-weight:700 !important; }
...
""")
        ...
        with ui.element('div').style('padding:20px; overflow-y:auto; height:100%; display:flex; flex-direction:column;'):
            with ui.element('div').style('display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;'):
                with ui.row().classes('items-center gap-2'):
                    ui.icon('my_location').style('color:#F97316; font-size:20px;')
                ...
                with ui.element('div').classes('coord-item'):
                    with ui.element('div').style('display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px;'):
                        with ui.element('div'):
                            ...
                            cl = ui.label(str(x) + ',  ' + str(y)).style('font-size:20px; font-weight:800; color:#F97316; font-family:monospace; display:block;')
                            coord_labels[key] = cl
    ...
    with ui.header(elevated=False).style(
        ...
        with ui.row().classes('items-center justify-between w-full h-full'):
            with ui.row().classes('items-center gap-2'):
                ui.icon('moped', size='sm').style('color:#F97316;')
                ui.html('<span style="font-size:17px;font-weight:800;color:#F97316;">Keeta</span>'
                        '<span style="font-size:17px;font-weight:300;color:#f1f5f9;margin-left:4px;">Automation</span>')
                ui.html('<span style="background:rgba(249,115,22,.15);border:1px solid rgba(249,115,22,.4);border-radius:5px;padding:2px 8px;font-size:11px;font-weight:700;color:#F97316;">v1.0</span>')
    ...
    with ui.left_drawer(value=True, bordered=False).style(
        ...
        with ui.element('div').style('position:absolute;bottom:16px;left:10px;right:10px;background:#131c2e;border:1px solid #1e2d45;border-radius:12px;padding:14px;'):
            ...
            ui.label('Keeta, do jeito certo.').style('font-size:11px;color:#6b7280;margin-top:4px;display:block;')
    ...
    with ui.element('div').style('padding:20px; width:100%; min-height:calc(100vh - 52px);'):
        ...
        with ui.element('div').style('display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;'):
            with ui.row().classes('items-center gap-3'):
                with ui.element('div').style('width:48px;height:48px;border-radius:12px;background:#1e293b;display:flex;align-items:center;justify-content:center;border:1px solid #2d3748;flex-shrink:0;'):
                    ui.icon('auto_fix_high').style('color:#F97316;font-size:22px;')
                with ui.element('div'):
                    ui.label('Painel de Automacao').style('font-size:24px;font-weight:700;color:#f1f5f9;display:block;')
                    ui.label('Automatize o envio dos seus itens Keeta de forma rapida e inteligente').style('font-size:13px;color:#64748b;display:block;margin-top:2px;')
        ...
        with ui.element('div').style('display:grid;grid-template-columns:minmax(0,1fr) 320px;gap:20px;width:100%;align-items:start;'):
            ...
            with ui.element('div').style('display:flex;flex-direction:column;gap:14px;'):
                ...
                with ui.element('div').classes('card'):
                    with ui.element('div').style('display:flex;align-items:center;gap:16px;margin-bottom:16px;'):
                        ...
                        with ui.element('div').style('flex:1;min-width:0;'):
                            ...
                            status_txt = ui.label('Aguardando comando...').style('font-size:20px;font-weight:700;color:#F97316;display:block;')
                ...
                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:14px;'):
                        ui.icon('tune').style('color:#F97316;font-size:18px;')
                ...
                with ui.element('div').classes('card'):
                    with ui.element('div').style('display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;'):
                        with ui.row().classes('items-center gap-2'):
                            ui.icon('terminal').style('color:#F97316;font-size:18px;')
            ...
            with ui.element('div').style('display:flex;flex-direction:column;gap:14px;'):
                ...
                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:14px;'):
                        ui.icon('speed').style('color:#F97316;font-size:18px;')
                    ...
                    with ui.element('div').style('margin-bottom:14px;'):
                        with ui.element('div').style('display:flex;justify-content:space-between;margin-bottom:6px;'):
                            ...
                            dup_val = ui.label('3.0s').style('font-size:13px;font-weight:700;color:#F97316;')
                    ...
                    with ui.element('div'):
                        with ui.element('div').style('display:flex;justify-content:space-between;margin-bottom:6px;'):
                            ...
                            env_val = ui.label('3.0s').style('font-size:13px;font-weight:700;color:#F97316;')
                        def on_env(e): state['delay_env']=e.value; env_val.set_text('{:.1f}s'.format(e.value))
                ...
                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                        ui.icon('summarize').style('color:#F97316;font-size:18px;')
                    ...
                    def info_row(lbl, key, color='#f1f5f9', init='--'):
                        with ui.element('div').classes('info-row'):
                            ...
                            file_info_ref[key] = v
                    info_row('Arquivo atual',      'nome',   '#F97316', 'pizza.txt')
                ...
                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                        ui.icon('bolt').style('color:#F97316;font-size:18px;')
    ...
    with ui.element('div').style('background:#0d1526;border-top:1px solid #1a2540;padding:10px 24px;display:flex;justify-content:space-between;align-items:center;'):
        ui.label('2025 Keeta Automation. Todos os direitos reservados.').style('font-size:12px;color:#4b5563;')
    ...
    def _preview_arquivo():
        ...
        try:
            ...
            with ui.dialog() as dlg, ui.card().style('min-width:480px;background:#131c2e;border:1px solid #1e2d45;'):
                with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                    ui.icon('preview').style('color:#F97316;')
                ...
                ui.button('Fechar', on_click=dlg.close).props('flat color=orange').style('margin-top:12px;')
            dlg.open()
        except Exception as e:
    ...
    def _editar_coord(key):
        ...
        with ui.dialog() as dlg, ui.card().style('min-width:300px;background:#131c2e;border:1px solid #1e2d45;'):
            with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                ui.icon('edit_location').style('color:#F97316;')
    ...
    def _sync_ui():
        ...
        if fase != prev_fase['v']:
            prev_fase['v'] = fase
            if fase=='rodando':
                status_txt.set_text('Automacao em andamento...'); status_txt.style('font-size:20px;font-weight:700;color:#F97316;display:block;')
                ...
                status_txt.set_text('Pausado -- aguardando retomada...'); status_txt.style('font-size:20px;font-weight:700;color:#ea6d10;display:block;')
                ...
                status_txt.set_text('Aguardando comando...'); status_txt.style('font-size:20px;font-weight:700;color:#F97316;display:block;')
    ...
    ui.run(title='Keeta Automation', port=8080, reload=False, favicon='')
