"""
Painel de Automacao iFood - NiceGUI  (v1 -- visual 99food exato + logica iFood)
"""
...
state = {
    ...
    'delay_dup':        4.0,
    'delay_salvar':     3.0,
    'tres_pontos':      (1644, 480),
    'duplicar_btn':     (1644, 510),
    'confirmar_dup':    (1124, 633),
    'verificar_preco':  (1501, 484),
    'campo_nome':       (613,  507),
    'prosseguir':       (1796, 979),
    'cat_preco':        (569,  648),
    'inicio_tempo':     None,
...
COORD_KEYS = [
    ('tres_pontos',     '3 Pontos (Abrir Menu)'),
    ('duplicar_btn',    'Btn Duplicar (no Menu)'),
    ('confirmar_dup',   'Confirmar Duplicacao'),
    ('verificar_preco', 'Verificar Preco'),
    ('campo_nome',      'Campo Nome'),
    ('prosseguir',      'Btn Prosseguir/Salvar'),
    ('cat_preco',       'Categoria/Preco'),
]
...
LOG_ICONS = {
    'inicio':    ('rocket_launch', '#EA1D2C'),
    'arquivo':   ('description',   '#94a3b8'),
    'item':      ('play_arrow',    '#EA1D2C'),
...
def _info_arquivo(path):
    try:
        stat  = os.stat(path)
        size  = '{:.1f} KB'.format(stat.st_size / 1024)
...
def _sleep_cancelavel(segundos):
...
# -- Thread de automacao (logica iFood -- nao alterar) ------------------------
def _thread_automacao():
    ...
    try:
        ...
        _push('separador')
        _push('inicio', 'Iniciando automacao iFood...')

        if not os.path.exists(arquivo):
            _push('erro', 'Arquivo nao encontrado: ' + arquivo)
            state['fase'] = 'parado'; stop_event.clear(); pause_event.set(); return

        with open(arquivo, 'r', encoding='utf-8') as f:
            linhas = [l for l in f.readlines() if l.strip()]

        _push('arquivo', str(len(linhas)) + ' itens encontrados em ' + os.path.basename(arquivo))
        _push('separador')
        ...
        for i, linha in enumerate(linhas, start=1):
            if stop_event.is_set():
                _push('parada', 'Automacao interrompida pelo usuario.'); break
            if not pause_event.is_set():
                _push('pausa', 'Pausado antes do item ' + str(i) + ' -- aguardando retomada...')
                if not _aguardar_se_pausado():
                    _push('parada', 'Parado durante pausa.'); break
                _push('retomada', 'Retomando automacao...')

            try:
                partes = linha.strip().split('|')
                if len(partes) != 3:
                    _push('aviso', 'Linha ' + str(i) + ' -- formato invalido (espera nome|descricao|preco): ' + linha.strip())
                    falhados += 1; continue
                ...
                _push('item', '[' + str(i) + '/' + str(len(linhas)) + ']  ' + nome + '  --  R$ ' + '{:.2f}'.format(preco_base))

                pyautogui.press('home')

                # --- Aguardar carregamento (verificar preco do 1o item) ---
                _push('info', 'Verificando carregamento da pagina...')
                pyperclip.copy('')
                pyautogui.moveTo(*state['verificar_preco'], duration=0.1)
                pyautogui.doubleClick()
                pyautogui.hotkey('ctrl', 'c')
                time.sleep(0.5)

                # --- Duplicar item base ---
                _push('info', 'Duplicando item base...')
                pyautogui.press('home')
                pyautogui.moveTo(*state['tres_pontos'], duration=0.1)
                pyautogui.click()
                time.sleep(0.5)
                pyautogui.moveTo(*state['duplicar_btn'], duration=0.1)
                pyautogui.click()
                if not _sleep_cancelavel(state['delay_dup']):
                    _push('parada', 'Parado durante delay de duplicacao.'); break
                pyautogui.moveTo(*state['confirmar_dup'], duration=0.1)
                pyautogui.click()
                time.sleep(2)

                # --- Preencher nome ---
                pyperclip.copy(nome)
                pyautogui.moveTo(*state['campo_nome'], duration=0.1)
                pyautogui.click()
                ...
                # --- Prosseguir (2x) ---
                pyautogui.moveTo(*state['prosseguir'], duration=0.1)
                pyautogui.click()
                pyautogui.click()
                time.sleep(1)

                # --- Preco ---
                pyautogui.moveTo(*state['cat_preco'], duration=0.1)
                pyautogui.click()
                for _ in range(2):
                    pyautogui.press('tab')
                preco_str = '{:.2f}'.format(preco_base).replace('.', ',')
                pyautogui.typewrite(preco_str)

                # --- Salvar ---
                pyautogui.moveTo(*state['prosseguir'], duration=0.1)
                pyautogui.click()

                if not _sleep_cancelavel(state['delay_salvar']):
                    _push('parada', 'Parado durante delay de salvamento.'); break

                _push('sucesso', '"' + nome + '" adicionado com sucesso')
                adicionados += 1; state['itens_ok'] = adicionados

            except Exception as e:
                _push('erro', 'Erro no item ' + str(i) + ': ' + str(e)); falhados += 1

        _push('separador')
        _push('resumo', 'Concluido -- ' + str(adicionados) + ' adicionados   ' + str(falhados) + ' falhados')
        _push('separador')
...
.dash-item { display:flex; align-items:center; gap:10px; padding:10px 14px; border-radius:9px; cursor:pointer; color:#94a3b8; font-size:13px; user-select:none; transition:all .15s; border-left:3px solid transparent; }
.dash-item:hover { background:rgba(255,255,255,.04); color:#cbd5e1; }
.dash-item.active { background:rgba(234,29,44,.1); border-left-color:#EA1D2C; color:#EA1D2C; font-weight:700; }
.nav-item { display:flex; align-items:center; gap:10px; padding:9px 14px; border-radius:9px; cursor:pointer; color:#94a3b8; font-size:13px; user-select:none; transition:all .15s; border-left:3px solid transparent; }
.nav-item:hover { background:rgba(255,255,255,.04); color:#cbd5e1; }
.nav-item.active { background:rgba(234,29,44,.1); border-left-color:#EA1D2C; color:#EA1D2C; font-weight:700; }
.file-item { display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:9px; cursor:pointer; color:#64748b; font-size:13px; user-select:none; transition:all .15s; }
.file-item.selected { background:#EA1D2C !important; color:#fff !important; font-weight:700; }
.file-item:not(.selected):hover { background:rgba(255,255,255,.04); color:#cbd5e1; }
.sec-lbl { font-size:10px; font-weight:800; letter-spacing:.12em; text-transform:uppercase; color:#374151; display:block; padding:0 14px; margin-bottom:4px; margin-top:2px; }
.spinner-ring { width:62px; height:62px; border-radius:50%; border:5px solid rgba(234,29,44,.15); border-top-color:#EA1D2C; flex-shrink:0; }
@keyframes spin { to { transform:rotate(360deg); } }
.spinner-ring.spinning { animation:spin .8s linear infinite; }
.spinner-ring.paused   { border-top-color:#C41825; animation:spin 2s linear infinite; }
.spinner-ring.stopped  { border-top-color:#ef4444; }
.stat-box { flex:1; min-width:0; background:#0f172a; border:1px solid #1e293b; border-radius:12px; padding:14px 14px; display:flex; align-items:center; gap:10px; }
.stat-icon { width:34px; height:34px; border-radius:9px; background:#1e293b; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
.prog-track { background:#1e293b; border-radius:4px; height:5px; width:100%; margin-top:5px; overflow:hidden; }
.prog-fill  { background:linear-gradient(90deg,#C41825,#EA1D2C); height:5px; width:0%; transition:width .4s; border-radius:4px; }
@keyframes pulse-glow { 0%,100% { box-shadow:0 4px 14px rgba(234,29,44,.3); } 50% { box-shadow:0 4px 26px rgba(234,29,44,.65); transform:translateY(-1px); } }
.btn-ini { flex:1; min-width:0; background:linear-gradient(135deg,#EA1D2C,#C41825) !important; color:#fff !important; font-weight:800 !important; border-radius:10px !important; padding:12px !important; text-transform:none !important; font-size:14px !important; animation:pulse-glow 2s ease-in-out infinite; transition:all .2s !important; }
...
.btn-ctrl { flex:1; min-width:0; background:#1e293b !important; border:1px solid #2d3748 !important; color:#cbd5e1 !important; font-weight:600 !important; border-radius:10px !important; padding:12px !important; text-transform:none !important; font-size:14px !important; transition:all .2s !important; }
...
.btn-resume { flex:1; min-width:0; background:linear-gradient(135deg,#16a34a,#15803d) !important; color:#fff !important; font-weight:700 !important; border-radius:10px !important; padding:12px !important; text-transform:none !important; font-size:14px !important; }
.btn-resume[disabled] { opacity:.35 !important; }
.btn-stop-sm { background:#ef4444 !important; color:#fff !important; font-weight:700 !important; border-radius:8px !important; padding:8px 16px !important; text-transform:none !important; font-size:13px !important; flex-shrink:0; }
.btn-stop-sm[disabled] { opacity:.35 !important; }
.btn-quick { width:100% !important; background:#1e293b !important; border:1px solid #2d3748 !important; color:#94a3b8 !important; border-radius:9px !important; text-transform:none !important; font-size:13px !important; padding:10px 14px !important; transition:all .15s !important; }
.btn-quick:hover { background:#243044 !important; color:#EA1D2C !important; border-color:rgba(234,29,44,.35) !important; }
...
.btn-cap-sm { flex:1; background:rgba(234,29,44,.12) !important; border:1px solid rgba(234,29,44,.4) !important; color:#EA1D2C !important; border-radius:7px !important; font-size:12px !important; text-transform:none !important; padding:6px 10px !important; font-weight:700 !important; }
...
""")
        ...
        with ui.element('div').style('padding:20px; overflow-y:auto; height:100%; display:flex; flex-direction:column;'):
            with ui.element('div').style('display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;'):
                with ui.row().classes('items-center gap-2'):
                    ui.icon('my_location').style('color:#EA1D2C; font-size:20px;')
            ...
            ui.label('Mova o mouse ate a posicao e clique em Capturar. A captura ocorre apos 2 segundos.').style('font-size:12px; color:#6b7280; line-height:1.6; display:block; margin-bottom:16px;')
                ...
                with ui.element('div').classes('coord-item'):
                    with ui.element('div').style('display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px;'):
                        with ui.element('div'):
                            ...
                            cl = ui.label(str(x) + ',  ' + str(y)).style('font-size:20px; font-weight:800; color:#EA1D2C; font-family:monospace; display:block;')
                            coord_labels[key] = cl
    ...
    with ui.header(elevated=False).style(
        ...
        with ui.row().classes('items-center justify-between w-full h-full'):
            with ui.row().classes('items-center gap-2'):
                ui.icon('fastfood', size='sm').style('color:#EA1D2C;')
                ui.html('<span style="font-size:17px;font-weight:800;color:#EA1D2C;">iFood</span>'
                        '<span style="font-size:17px;font-weight:300;color:#f1f5f9;margin-left:4px;">Automation</span>')
                ui.html('<span style="background:rgba(234,29,44,.15);border:1px solid rgba(234,29,44,.4);border-radius:5px;padding:2px 8px;font-size:11px;font-weight:700;color:#EA1D2C;">v1.0</span>')
            with ui.row().classes('items-center gap-3'):
                ui.icon('light_mode',size='xs').style('color:#6b7280;')
                ui.switch('',value=True,on_change=lambda e: dark.enable() if e.value else dark.disable()).props('color=red dense')
                ...
                with ui.row().classes('items-center gap-2').style('cursor:pointer;'):
                    with ui.element('div').style('width:32px;height:32px;border-radius:50%;background:#374151;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:13px;'):
                        ui.label('A')
    ...
    with ui.left_drawer(value=True, bordered=False).style(
        ...
        ui.label('CONFIGURACOES').classes('sec-lbl')
        ...
        with ui.element('div').classes('nav-item'):
            ui.icon('tune',size='xs').style('color:inherit;'); ui.label('Preferencias').style('color:inherit;font-size:13px;')
        ui.element('div').style('height:10px;')
        ...
        with ui.element('div').classes('nav-item'):
            ui.icon('history',size='xs').style('color:inherit;'); ui.label('Historico').style('color:inherit;font-size:13px;')
        ...
        with ui.element('div').style('position:absolute;bottom:16px;left:10px;right:10px;background:#131c2e;border:1px solid #1e2d45;border-radius:12px;padding:14px;'):
            ...
            ui.label('iFood, do jeito certo.').style('font-size:11px;color:#6b7280;margin-top:4px;display:block;')


    with ui.element('div').style('padding:20px; width:100%; min-height:calc(100vh - 52px);'):
        ...
        with ui.element('div').style('display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;'):
            with ui.row().classes('items-center gap-3'):
                with ui.element('div').style('width:48px;height:48px;border-radius:12px;background:#1e293b;display:flex;align-items:center;justify-content:center;border:1px solid #2d3748;flex-shrink:0;'):
                    ui.icon('auto_fix_high').style('color:#EA1D2C;font-size:22px;')
                with ui.element('div'):
                    ui.label('Painel de Automacao').style('font-size:24px;font-weight:700;color:#f1f5f9;display:block;')
                    ui.label('Automatize o envio dos seus itens iFood de forma rapida e inteligente').style('font-size:13px;color:#64748b;display:block;margin-top:2px;')
        ...
        with ui.element('div').style('display:grid;grid-template-columns:minmax(0,1fr) 320px;gap:20px;width:100%;align-items:start;'):
            ...
            with ui.element('div').style('display:flex;flex-direction:column;gap:14px;'):
                ...
                with ui.element('div').classes('card'):
                    with ui.element('div').style('display:flex;align-items:center;gap:16px;margin-bottom:16px;'):
                        ...
                        with ui.element('div').style('flex:1;min-width:0;'):
                            ...
                            status_txt = ui.label('Aguardando comando...').style('font-size:20px;font-weight:700;color:#EA1D2C;display:block;')
                    ...
                    with ui.element('div').style('display:flex;gap:10px;'):
                        ...
                        _stat('show_chart',  'Medias',    'media_lbl', '--')
                        _stat('trending_up', 'Progresso', 'prog_lbl',  '0%', has_bar=True)

                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:14px;'):
                        ui.icon('tune').style('color:#EA1D2C;font-size:18px;')
                        ui.label('Controles de Execucao').style('font-size:14px;font-weight:700;color:#f1f5f9;')
                    with ui.element('div').style('display:flex;gap:10px;'):
                ...
                with ui.element('div').classes('card'):
                    with ui.element('div').style('display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;'):
                        with ui.row().classes('items-center gap-2'):
                            ui.icon('terminal').style('color:#EA1D2C;font-size:18px;')
                            ui.label('Log de Execucao').style('font-size:14px;font-weight:700;color:#f1f5f9;')
                    ...
                    _push('info', 'iFood Automation Panel -- Pronto para iniciar')

            with ui.element('div').style('display:flex;flex-direction:column;gap:14px;'):

                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:14px;'):
                        ui.icon('speed').style('color:#EA1D2C;font-size:18px;')
                        ui.label('Velocidade de Execucao').style('font-size:14px;font-weight:700;color:#f1f5f9;')
                    with ui.element('div').style('margin-bottom:14px;'):
                        with ui.element('div').style('display:flex;justify-content:space-between;margin-bottom:6px;'):
                            ui.label('Delay de duplicacao').style('font-size:13px;color:#94a3b8;')
                            dup_val = ui.label('4.0s').style('font-size:13px;font-weight:700;color:#EA1D2C;')
                        def on_dup(e): state['delay_dup']=e.value; dup_val.set_text('{:.1f}s'.format(e.value))
                        ui.slider(min=1,max=10,step=0.5,value=4.0,on_change=on_dup).props('color=red').classes('w-full')
                    with ui.element('div'):
                        with ui.element('div').style('display:flex;justify-content:space-between;margin-bottom:6px;'):
                            ui.label('Delay de salvamento').style('font-size:13px;color:#94a3b8;')
                            env_val = ui.label('3.0s').style('font-size:13px;font-weight:700;color:#EA1D2C;')
                        def on_env(e): state['delay_salvar']=e.value; env_val.set_text('{:.1f}s'.format(e.value))
                        ui.slider(min=1,max=8,step=0.5,value=3.0,on_change=on_env).props('color=red').classes('w-full')

                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                        ui.icon('summarize').style('color:#EA1D2C;font-size:18px;')
                    ...
                    def info_row(lbl, key, color='#f1f5f9', init='--'):
                        with ui.element('div').classes('info-row'):
                            ...
                            v = ui.label(init).style('font-size:13px;font-weight:600;color:' + color + ';text-align:right;')
                            file_info_ref[key] = v
                    info_row('Arquivo atual',      'nome',   '#EA1D2C', 'pizza.txt')
                    info_row('Tamanho',            'size')
                    info_row('Ultima modificacao', 'mtime')
                    info_row('Linhas',             'lines')
                    info_row('Status',             'status', '#22c55e', '--')

                with ui.element('div').classes('card'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                        ui.icon('bolt').style('color:#EA1D2C;font-size:18px;')
                        ui.label('Comandos Rapidos').style('font-size:14px;font-weight:700;color:#f1f5f9;')
                    with ui.element('div').style('display:flex;flex-direction:column;gap:8px;'):
                        with ui.button(on_click=lambda: _testar_conexao()).classes('btn-quick'):
                            with ui.row().classes('items-center gap-2 w-full justify-center'):
                                ui.icon('wifi',size='xs'); ui.label('Testar Conexao')
                        with ui.button(on_click=lambda: _preview_arquivo()).classes('btn-quick'):
                            with ui.row().classes('items-center gap-2 w-full justify-center'):
                                ui.icon('visibility',size='xs'); ui.label('Pre-visualizar Arquivo')
    ...
    with ui.element('div').style('background:#0d1526;border-top:1px solid #1a2540;padding:10px 24px;display:flex;justify-content:space-between;align-items:center;'):
        ui.label('2025 iFood Automation. Todos os direitos reservados.').style('font-size:12px;color:#4b5563;')
    ...
    def _atualizar_resumo():
        ...
        file_info_ref['status'].set_text('Carregado' if ok else 'Nao encontrado')
        color = '#22c55e' if ok else '#ef4444'
        file_info_ref['status'].style('font-size:13px;font-weight:600;color:' + color + ';text-align:right;')
    ...
    def _testar_conexao():
        try:
            import pyautogui
            ui.notify('pyautogui OK -- automacao pronta!', type='positive', position='top-right', timeout=4000)
        except Exception:
            ui.notify('pyautogui nao encontrado', type='negative', position='top-right', timeout=4000)
    ...
    def _preview_arquivo():
        ...
        try:
            ...
            with ui.dialog() as dlg, ui.card().style('min-width:480px;background:#131c2e;border:1px solid #1e2d45;'):
                with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                    ui.icon('preview').style('color:#EA1D2C;')
                    ui.label('Previa -- ' + os.path.basename(path)).style('font-weight:700;font-size:15px;')
                with ui.element('div').style('background:#080e1a;border-radius:8px;padding:14px;max-height:300px;overflow-y:auto;'):
                    for i, l in enumerate(linhas, 1):
                        ui.label(str(i).zfill(2) + '. ' + l).style('font-family:monospace;font-size:12px;color:#94a3b8;display:block;padding:2px 0;')
                ui.button('Fechar', on_click=dlg.close).props('flat color=red').style('margin-top:12px;')
            dlg.open()
        except Exception as e:
            ui.notify('Erro: ' + str(e), type='negative')
    ...
    def _capturar_coord(key):
        ...
        if btn: btn.disable()
        _push('aviso', 'Mova o mouse para a posicao...')
        def _run():
            for i in range(2, 0, -1):
                _push('info', 'Capturando em ' + str(i) + 's...'); time.sleep(1.0)
            x, y = pyautogui.position()
            coord_queue.put(('update', key, x, y))
            coord_queue.put(('enable_btn', key))
            _salvar_coordenadas()
            _push('sucesso', '"' + key + '" -> (' + str(x) + ', ' + str(y) + ') salvo')
    ...
    def _editar_coord(key):
        ...
        with ui.dialog() as dlg, ui.card().style('min-width:300px;background:#131c2e;border:1px solid #1e2d45;'):
            with ui.row().classes('items-center gap-2').style('margin-bottom:12px;'):
                ui.icon('edit_location').style('color:#EA1D2C;')
                ui.label('Editar -- ' + lbl_txt).style('font-weight:700;font-size:15px;')
            ...
            with ui.row().classes('gap-2 justify-end').style('margin-top:12px;'):
                ...
                def salvar():
                    ...
                    _push('info', '"' + key + '" -> (' + str(nx) + ', ' + str(ny) + ')'); dlg.close()
                ui.button('Salvar', on_click=salvar).props('color=red')
        dlg.open()
    ...
    def _sync_ui():
                ...
                with log_ref['container']:
                    if tipo == 'separador':
                        ...
                        with ui.element('div').classes('log-line'):
                            ui.label('[' + ts + ']').classes('log-ts')
                            ui.icon(icon_nm,size='xs').style('color:' + color + ';flex-shrink:0;margin-top:3px;')
                            ui.label(msg).classes('log-msg').style('color:' + tc + ';')
        ...
        while not notif_queue.empty():
            try:
                ...
                if tipo_n == 'sucesso':
                    ui.notify('Concluido! ' + str(ad) + ' itens adicionados.', type='positive', position='top', timeout=8000)
                else:
                    ui.notify(str(ad) + ' adicionados, ' + str(fa) + ' falhados.', type='warning', position='top', timeout=8000)
            except Exception:
                pass
        ...
        while not coord_queue.empty():
            try:
                ...
                if msg[0]=='update':
                    ...
                    if key in coord_labels: coord_labels[key].set_text(str(x) + ',  ' + str(y))
        ...
        if state['inicio_tempo'] is not None:
            ...
            ui_refs['tempo_lbl'].set_text(str(h).zfill(2)+':'+str(m).zfill(2)+':'+str(s).zfill(2))
            itens=state['itens_ok']; total=state['itens_total']
            ui_refs['itens_lbl'].set_text(str(itens))
            if elapsed>=5 and itens>0: ui_refs['media_lbl'].set_text('{:.1f}/min'.format(itens/(elapsed/60)))
            pct=int(itens/total*100) if total>0 else 0
            ui_refs['prog_lbl'].set_text(str(pct)+'%')
            ui.run_javascript('var b=document.getElementById("prog-bar");if(b)b.style.width="' + str(pct) + '%"')

        fase = state['fase']
        if fase != prev_fase['v']:
            prev_fase['v'] = fase
            if fase=='rodando':
                status_txt.set_text('Automacao em andamento...'); status_txt.style('font-size:20px;font-weight:700;color:#EA1D2C;display:block;')
                status_sidebar_lbl.set_text('Em execucao')
                ...
                status_txt.set_text('Pausado -- aguardando retomada...'); status_txt.style('font-size:20px;font-weight:700;color:#C41825;display:block;')
                ...
                status_txt.set_text('Aguardando comando...'); status_txt.style('font-size:20px;font-weight:700;color:#EA1D2C;display:block;')
    ...
    def _parar():
        ...
        btn_pau.classes(remove='btn-resume',add='btn-ctrl'); btn_pau.clear()
        with btn_pau: ui.icon('pause',size='sm'); ui.label('Pausar')
        _push('parada','Encerrando apos o item atual...')

    def _limpar_log():
        log_ref['container'].clear()

    ui.run(title='iFood Automation', port=8082, reload=False, favicon='')
